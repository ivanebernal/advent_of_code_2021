...
lines.forEach { line ->
    ...
    if(start.first != end.first && start.second != end.second) {
        var x = start.first
        var y = start.second
        val dx = if(start.first > end.first) - 1 else 1
        val dy = if(start.second > end.second) -1 else 1

        // We can check one of the axis because diagonals are guaranteed to be 45 degrees
        while(if(dx > 0) x <= end.first else x >= end.first) {
            val point = Pair(x, y)
