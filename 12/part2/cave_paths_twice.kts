...
var smallTwice = mutableSetOf<Node>()
var paths = 0
...
fun visit(cave: Node) {
    ...
    if((smallVisited.contains(cave) && smallTwice.isNotEmpty()) || cave == getNode("start")) {
    ...
    if(smallTwice.contains(cave)) {
        smallTwice.remove(cave)
    } else if (smallVisited.contains(cave)) {
